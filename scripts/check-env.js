...
if (allSet) {
  ...
  console.log("If you're still having issues, check your Firebase project settings.");
} else {
  console.log("❌ Some Firebase environment variables are missing!");
  console.log("\nMake sure you have a .env.local file with:");
  requiredVars.forEach(varName => {
    console.log(`${varName}=your_${varName.toLowerCase().replace(/next_public_firebase_/, '').replace(/_/g, '_')}`);
  });
