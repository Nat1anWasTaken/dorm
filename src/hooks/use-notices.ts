...
export const noticesKeys = {
  ...
  infinite: (params?: unknown) => ["notices", "infinite", params] as const,
...
export function useInfiniteNotices(params?: {
  category?: string;
  search?: string;
  pageSize?: number;
  pinned?: boolean;
  ...
  const infiniteQuery = useInfiniteQuery({
    queryKey: noticesKeys.infinite(params),
    queryFn: ({ pageParam }) =>
      noticeApi.fetchNoticesInfinite({
        ...params,
        limit: params?.pageSize || 10,
        lastNoticeId: pageParam,
      }),
  ...
  const notices = infiniteQuery.data?.pages.flatMap(page => page.notices) ?? [];
  ...
  return {
    ...
    error: (infiniteQuery.error as Error | null)?.message ?? null,
    fetchNextPage: infiniteQuery.fetchNextPage,
    refreshNotices: infiniteQuery.refetch,
    createNotice: createMutation.mutateAsync,
    updateNotice: (
      id: string,
      data: Parameters<typeof noticeApi.updateNotice>[1]
